...
def main():
    ...
    sim_batch_size = 1
    sim_input_shape = [sim_batch_size, 3, 32, 32]
    sim_input = torch.from_numpy(np.zeros(sim_input_shape).astype(np.float32))

    fc_flag = True
    layer_input = sim_input
    for id, layer in enumerate(model_layer_org):
        if type(layer).__name__ == 'Linear' and fc_flag:
            fc_flag = False
            layer_input = torch.reshape(layer_input,
                                        [-1, layer_input.shape[1] * layer_input.shape[2] * layer_input.shape[3]])
            # print(input.shape)
        # model_input[layer] = input.shape
        # print(layer_input)
        model_input.append(layer_input.shape)
        layer_output = layer(layer_input.cuda())
        layer_input = layer_output
        # print(output.shape)
        # model_output[layer] = output.shape
        model_output.append(layer_output.shape)

    model = convert_model.convert_model(model, model_input, model_output, model_layer_id)
    print(model)
    summary(model.cuda(), input_size=(3, 32, 32))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        }, is_best, filename=os.path.join(args.save_dir, 'checkpoint_best.tar'))
    save_checkpoint({
                'epoch': epoch + 1,
                'state_dict': model.state_dict(),
                'best_prec1': best_prec1,
            }, is_best=True, filename=os.path.join(args.save_dir, 'checkpoint_{}.tar'.format(epoch)))
